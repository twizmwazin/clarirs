...
fn peel_lhs_to_rhs<'c>(constraint: &BoolAst<'c>) -> Result<BoolAst<'c>, ClarirsError> {
    ...
    let (new_lhs, new_rhs, flip_sign): (BitVecAst<'c>, BitVecAst<'c>, bool) = match lhs.op() {
        ...
        BitVecOp::Annotated(inner, _) => {
            // Just remove the annotation and continue with the inner expression
            (inner.clone(), rhs.clone(), false)
        }
        // BitVecOp::SI(_, big_uint, big_uint1, big_uint2) => todo!(),
        ...
        |ast, children| match ast {
            DynAst::Boolean(bool_node) => {
                match bool_node.op() {
                    ...
                    BooleanOp::Annotated(..) => {
                        // For annotations, just pass through the child
                        Ok(children.first().cloned().flatten())
                    }
                    _ => Ok(None),
                }
            }
...
pub fn constraint_to_si<'c>(
    ...
    let constraint_ = constraint.deannotate()?.simplify()?;
