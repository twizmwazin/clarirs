...
impl<'c> Solver<'c> for Z3Solver<'c> {
    ...
    fn eval_n(&mut self, expr: &AstRef<'c>, n: u32) -> Result<Vec<AstRef<'c>>, ClarirsError> {
        ...
        for _ in 0..n {
            ...
            // Convert FP bit patterns back to float constants.
            let solution = match (&fp_sort, solution.op()) {
                (Some(fsort), AstOp::BVV(bv)) => {
                    let value = if *fsort == FSort::f32() {
                        Float::F32(f32::from_bits(u32::try_from(bv.to_biguint()).unwrap_or(0)))
                    } else {
                        Float::F64(f64::from_bits(u64::try_from(bv.to_biguint()).unwrap_or(0)))
                    };
                    ctx.fpv(value)?
                }
                _ => solution,
