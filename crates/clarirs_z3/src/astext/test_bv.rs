...
mod to_z3 {
    ...
    fn value_8bit() {
        let ctx = Context::new();
        let bv = ctx.bvv(BitVec::from(42u8)).unwrap();
        let z3_ast = bv.to_z3().unwrap();
    ...
    fn value_32bit() {
        let ctx = Context::new();
        let bv = ctx.bvv(BitVec::from(0xDEADBEEFu32)).unwrap();
        let z3_ast = bv.to_z3().unwrap();
        assert_eq!(z3_ast.decl_kind(), z3::DeclKind::Bnum);
    }
    ...
    fn value_64bit() {
        let ctx = Context::new();
        let bv = ctx.bvv(BitVec::from(0x0123456789ABCDEFu64)).unwrap();
        let z3_ast = bv.to_z3().unwrap();
        assert_eq!(z3_ast.decl_kind(), z3::DeclKind::Bnum);
    }
...
mod from_z3 {
    ...
    fn value_8bit() {
        ...
        let result = AstRef::from_z3(&ctx, z3_ast).unwrap();
        assert_eq!(result, ctx.bvv(BitVec::from(42u8)).unwrap());
    }
    ...
    fn value_32bit() {
        ...
        let result = AstRef::from_z3(&ctx, z3_ast).unwrap();
        assert_eq!(result, ctx.bvv(BitVec::from(0xDEADBEEFu32)).unwrap());
    }
    ...
    fn value_64bit() {
        ...
        let result = AstRef::from_z3(&ctx, z3_ast).unwrap();
        assert_eq!(result, ctx.bvv(BitVec::from(0x0123456789ABCDEFu64)).unwrap());
    }
...
mod roundtrip {
    ...
    fn value_8bit() {
        let ctx = Context::new();
        let ast = ctx.bvv(BitVec::from(42u8)).unwrap();
        assert_eq!(ast, round_trip(&ctx, &ast).unwrap());
    }
    ...
    fn value_32bit() {
        let ctx = Context::new();
        let ast = ctx.bvv(BitVec::from(0xDEADBEEFu32)).unwrap();
        assert_eq!(ast, round_trip(&ctx, &ast).unwrap());
    }
    ...
    fn value_64bit() {
        let ctx = Context::new();
        let ast = ctx.bvv(BitVec::from(0x0123456789ABCDEFu64)).unwrap();
        assert_eq!(ast, round_trip(&ctx, &ast).unwrap());
    }
    ...
    fn str_index_of_symbols() {
        ...
        let offset = ctx.bvv(BitVec::from(0u64)).unwrap();
    ...
    fn str_index_of_with_offset() {
        ...
        let s = ctx.strings("s").unwrap();
        let t = ctx.strings("t").unwrap();
        let offset = ctx.bvv(BitVec::from(5u64)).unwrap();
        let ast = ctx.str_index_of(s, t, offset).unwrap();
        assert_eq!(ast, round_trip(&ctx, &ast).unwrap());
    }
