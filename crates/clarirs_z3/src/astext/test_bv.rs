...
mod from_z3 {
    ...
    fn symbol() {
        let ctx = Context::new();
        let z3_ast = { Dynamic::from(BV::new_const("x", 32)) };
    ...
    fn value_8bit() {
        let ctx = Context::new();
        let z3_ast = { Dynamic::from(BV::from_u64(42, 8)) };
        let result = BitVecAst::from_z3(&ctx, z3_ast).unwrap();
