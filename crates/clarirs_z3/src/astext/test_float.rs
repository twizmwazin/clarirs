...
mod to_z3 {
    ...
    fn value_f32() {
        ...
        assert_eq!(z3_ast.decl_kind(), DeclKind::FPA_NUM);
    }
    ...
    fn value_f32_zero() {
        ...
        assert!(dk == DeclKind::FPA_NUM || dk == DeclKind::FPA_PLUS_ZERO);
    }
    ...
    fn value_f32_neg_zero() {
        ...
        let z3_ast = f.to_z3().unwrap();
        let dk = z3_ast.decl_kind();
        assert!(dk == DeclKind::FPA_NUM || dk == DeclKind::FPA_MINUS_ZERO);
    }
    ...
    fn fp_neg() {
        ...
        assert_eq!(z3_ast.decl_kind(), DeclKind::FPA_NEG);
    ...
    fn fp_abs() {
        ...
        assert_eq!(z3_ast.decl_kind(), DeclKind::FPA_ABS);
        assert_eq!(z3_ast.num_args(), 1);
        assert_eq!(z3_ast.arg(0).unwrap().symbol_name().as_deref(), Some("x"));
    }
    ...
    fn fp_sub() {
        ...
        assert_eq!(z3_ast.decl_kind(), DeclKind::FPA_SUB);
        assert_eq!(z3_ast.num_args(), 3);
        assert_eq!(
            z3_ast.arg(0).unwrap().decl_kind(),
            DeclKind::FPA_RM_TOWARD_ZERO
        );
    }
    ...
    fn fp_mul() {
        ...
        assert_eq!(z3_ast.decl_kind(), DeclKind::FPA_MUL);
        assert_eq!(z3_ast.num_args(), 3);
        assert_eq!(
            z3_ast.arg(0).unwrap().decl_kind(),
            DeclKind::FPA_RM_TOWARD_POSITIVE
        );
    }
    ...
    fn fp_div() {
        ...
        assert_eq!(z3_ast.decl_kind(), DeclKind::FPA_DIV);
        assert_eq!(z3_ast.num_args(), 3);
        assert_eq!(
            z3_ast.arg(0).unwrap().decl_kind(),
            DeclKind::FPA_RM_TOWARD_NEGATIVE
        );
    }
    ...
    fn fp_sqrt() {
        ...
        assert_eq!(z3_ast.decl_kind(), DeclKind::FPA_SQRT);
        // 2 args: rounding mode, operand
        assert_eq!(z3_ast.num_args(), 2);
        assert_eq!(
            z3_ast.arg(0).unwrap().decl_kind(),
            DeclKind::FPA_RM_NEAREST_TIES_TO_AWAY
        );
    ...
    fn fp_to_fp() {
        ...
        assert_eq!(z3_ast.decl_kind(), DeclKind::FPA_TO_FP);
        // 2 args: rounding mode, operand
        assert_eq!(z3_ast.num_args(), 2);
    }
    ...
    fn bv_to_fp_unsigned() {
        ...
        assert_eq!(z3_ast.decl_kind(), DeclKind::FPA_TO_FP_UNSIGNED);
        assert_eq!(z3_ast.num_args(), 2);
    }
    ...
    fn fp_fp() {
        ...
        assert_eq!(z3_ast.decl_kind(), DeclKind::FPA_FP);
        assert_eq!(z3_ast.num_args(), 3);
    }
...
mod from_z3 {
    ...
    fn fp_neg() {
        ...
        let z3_neg = Z3_CONTEXT.with(|&z3_ctx| unsafe {
            RcAst::try_from(Z3_mk_fpa_neg(z3_ctx, *x).expect("Z3_mk_fpa_neg returned null"))
                .unwrap()
        });
    ...
    fn fp_abs() {
        let ctx = Context::new();
        let x = RcAst::mk_fp("x", FSort::f32());
        let z3_abs = Z3_CONTEXT.with(|&z3_ctx| unsafe {
            RcAst::try_from(Z3_mk_fpa_abs(z3_ctx, *x).expect("Z3_mk_fpa_abs returned null"))
                .unwrap()
        });
    ...
    fn fp_add() {
        ...
        let z3_add = Z3_CONTEXT.with(|&z3_ctx| unsafe {
            RcAst::try_from(
                Z3_mk_fpa_add(z3_ctx, *rm, *a, *b).expect("Z3_mk_fpa_add returned null"),
            )
            .unwrap()
        });
    ...
    fn fp_sub() {
        ...
        let z3_sub = Z3_CONTEXT.with(|&z3_ctx| unsafe {
            RcAst::try_from(
                Z3_mk_fpa_sub(z3_ctx, *rm, *a, *b).expect("Z3_mk_fpa_sub returned null"),
            )
            .unwrap()
        });
    ...
    fn fp_mul() {
        ...
        let z3_mul = Z3_CONTEXT.with(|&z3_ctx| unsafe {
            RcAst::try_from(
                Z3_mk_fpa_mul(z3_ctx, *rm, *a, *b).expect("Z3_mk_fpa_mul returned null"),
            )
            .unwrap()
        });
    ...
    fn fp_div() {
        ...
        let z3_div = Z3_CONTEXT.with(|&z3_ctx| unsafe {
            RcAst::try_from(
                Z3_mk_fpa_div(z3_ctx, *rm, *a, *b).expect("Z3_mk_fpa_div returned null"),
            )
            .unwrap()
        });
    ...
    fn fp_sqrt() {
        ...
        let z3_sqrt = Z3_CONTEXT.with(|&z3_ctx| unsafe {
            RcAst::try_from(Z3_mk_fpa_sqrt(z3_ctx, *rm, *x).expect("Z3_mk_fpa_sqrt returned null"))
                .unwrap()
        });
    ...
    fn ite() {
        ...
        let z3_ite = Z3_CONTEXT.with(|&z3_ctx| unsafe {
            RcAst::try_from(Z3_mk_ite(z3_ctx, *c, *a, *b).expect("Z3_mk_ite returned null"))
                .unwrap()
        });
