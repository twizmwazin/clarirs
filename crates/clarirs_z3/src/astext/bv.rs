    ...
    Z3_CONTEXT.with(|&z3_ctx| unsafe {
        Ok(match ast.op() {
            BitVecOp::BVS(s, w) => {
                ...
                let sort = Z3_mk_bv_sort(z3_ctx, *w).expect("Z3_mk_bv_sort returned null");
                RcAst::try_from(Z3_mk_const(z3_ctx, sym, sort).expect("Z3_mk_const returned null"))?
            }
            BitVecOp::BVV(v) => {
                let sort = Z3_mk_bv_sort(z3_ctx, v.len()).expect("Z3_mk_bv_sort returned null");
                let numeral = v.to_biguint().to_string();
                let numeral_cstr = std::ffi::CString::new(numeral).unwrap();
                RcAst::try_from(
                    Z3_mk_numeral(z3_ctx, numeral_cstr.as_ptr(), sort)
            ...
            BitVecOp::ZeroExt(_, i) => RcAst::try_from(
                Z3_mk_zero_ext(z3_ctx, *i, **child(children, 0)?)
                    .expect("Z3_mk_zero_ext returned null"),
            )?,
            BitVecOp::SignExt(_, i) => RcAst::try_from(
                Z3_mk_sign_ext(z3_ctx, *i, **child(children, 0)?)
                    .expect("Z3_mk_sign_ext returned null"),
            )?,
            BitVecOp::Extract(a, high, low) => {
                ...
                RcAst::try_from(
                    Z3_mk_extract(z3_ctx, *high, *low, **a).expect("Z3_mk_extract returned null"),
                )?
            }
            BitVecOp::Concat(args) => {
                ...
                for i in 1..children.len() {
                    result = RcAst::try_from(
                        Z3_mk_concat(z3_ctx, *result, **child(children, i)?)
                            .expect("Z3_mk_concat returned null"),
                    )?;
                }
                result
            }
            BitVecOp::ByteReverse(a) => {
                ...
                let mut result = RcAst::try_from(
                    Z3_mk_extract(z3_ctx, 7, 0, **child_z3).expect("Z3_mk_extract returned null"),
                )?;
                ...
                for i in 1..num_bytes {
                    let high = (i + 1) * 8 - 1;
                    let low = i * 8;
                    let byte = RcAst::try_from(
                        Z3_mk_extract(z3_ctx, high, low, **child_z3)
                            .expect("Z3_mk_extract returned null"),
                    )?;
                    result = RcAst::try_from(
                        Z3_mk_concat(z3_ctx, *result, *byte).expect("Z3_mk_concat returned null"),
                    )?;
                }
            ...
            BitVecOp::FpToIEEEBV(..) => {
                let a = child(children, 0)?;
                RcAst::try_from(
                    Z3_mk_fpa_to_ieee_bv(z3_ctx, **a).expect("Z3_mk_fpa_to_ieee_bv returned null"),
                )?
            }
            BitVecOp::FpToUBV(_, size, rm) => {
                ...
                let a = child(children, 0)?;
                RcAst::try_from(
                    Z3_mk_fpa_to_ubv(z3_ctx, *rm_ast, **a, *size)
            ...
            BitVecOp::FpToSBV(_, size, rm) => {
                ...
                let a = child(children, 0)?;
                RcAst::try_from(
                    Z3_mk_fpa_to_sbv(z3_ctx, *rm_ast, **a, *size)
                        .expect("Z3_mk_fpa_to_sbv returned null"),
                )?
            }
            BitVecOp::StrLen(..) => {
                let a = child(children, 0)?;
                let str_len = RcAst::try_from(
                    Z3_mk_seq_length(z3_ctx, **a).expect("Z3_mk_seq_length returned null"),
                )?;
                RcAst::try_from(
                    Z3_mk_int2bv(z3_ctx, 64, *str_len).expect("Z3_mk_int2bv returned null"),
                )?
            }
            BitVecOp::StrIndexOf(..) => {
                ...
                let offset_int = RcAst::try_from(
                    Z3_mk_bv2int(z3_ctx, **offset_bv, false).expect("Z3_mk_bv2int returned null"),
                )?;
                let index_int = RcAst::try_from(
                    Z3_mk_seq_index(z3_ctx, **haystack, **needle, *offset_int)
                        .expect("Z3_mk_seq_index returned null"),
                )?;
                RcAst::try_from(
                    Z3_mk_int2bv(z3_ctx, 64, *index_int).expect("Z3_mk_int2bv returned null"),
                )?
            }
            BitVecOp::StrToBV(..) => {
                let a = child(children, 0)?;
                let int_val = RcAst::try_from(
                    Z3_mk_str_to_int(z3_ctx, **a).expect("Z3_mk_str_to_int returned null"),
                )?;
                RcAst::try_from(
                    Z3_mk_int2bv(z3_ctx, 64, *int_val).expect("Z3_mk_int2bv returned null"),
                )?
            }
    ...
    Z3_CONTEXT.with(|z3_ctx| unsafe {
        ...
        match ast_kind {
            AstKind::Numeral => {
                ...
                let sort_num = Z3_get_bv_sort_size(*z3_ctx, sort);
            ...
            AstKind::App => {
                ...
                match decl_kind {
                    ...
                    | DeclKind::BMUL => {
                        ...
                        match decl_kind {
                            DeclKind::BAND => ctx.bv_and_many(args),
                            DeclKind::BOR => ctx.bv_or_many(args),
                            DeclKind::BXOR => ctx.bv_xor_many(args),
                            DeclKind::BADD => ctx.add_many(args),
                            DeclKind::BMUL => ctx.mul_many(args),
                            _ => unreachable!(),
                        }
                    }
                    DeclKind::BSUB
                    ...
                    | DeclKind::BASHR => {
                        ...
                        match decl_kind {
                            DeclKind::BSUB => ctx.sub(a, b),
                            DeclKind::BUDIV => ctx.udiv(a, b),
                            DeclKind::BSDIV => ctx.sdiv(a, b),
                            DeclKind::BUREM => ctx.urem(a, b),
                            DeclKind::BSREM => ctx.srem(a, b),
                            DeclKind::BSHL => ctx.shl(a, b),
                            DeclKind::BLSHR => ctx.lshr(a, b),
                            DeclKind::BASHR => ctx.ashr(a, b),
                            _ => unreachable!(),
                        }
                    }
                    DeclKind::EXT_ROTATE_LEFT | DeclKind::EXT_ROTATE_RIGHT => {
                        let arg0 = RcAst::try_from(
                            Z3_get_app_arg(*z3_ctx, app, 0).expect("Z3_get_app_arg returned null"),
                        )?;
                        let arg1 = RcAst::try_from(
                        ...
                        let b = BitVecAst::from_z3(ctx, arg1)?;
                        match decl_kind {
                            DeclKind::EXT_ROTATE_LEFT => ctx.rotate_left(a, b),
                            DeclKind::EXT_ROTATE_RIGHT => ctx.rotate_right(a, b),
                            _ => unreachable!(),
                        }
                    }
                    DeclKind::ZERO_EXT | DeclKind::SIGN_EXT => {
                        ...
                        let inner = BitVecAst::from_z3(ctx, arg)?;
                        let i = Z3_get_decl_int_parameter(*z3_ctx, decl, 0) as u32;
                        match decl_kind {
                            DeclKind::ZERO_EXT => ctx.zero_ext(&inner, i),
                            DeclKind::SIGN_EXT => ctx.sign_ext(inner, i),
                            _ => unreachable!(),
                        }
                    }
                    DeclKind::EXTRACT => {
                    ...
                    DeclKind::INT2BV => {
                        let inner_int =
                            Z3_get_app_arg(*z3_ctx, app, 0).expect("Z3_get_app_arg returned null");
                        let inner_ast_kind = Z3_get_ast_kind(*z3_ctx, *ast);
                        match inner_ast_kind {
                            AstKind::Numeral => {
                                ...
                                let sort = Z3_get_sort(*z3_ctx, inner_int)
                                    .expect("Z3_get_sort returned null");
                                let sort_num = Z3_get_bv_sort_size(*z3_ctx, sort);
                                let biguint = BitVec::from_str(numeral_str, sort_num).unwrap();
                                ctx.bvv(biguint)
                            }
                            AstKind::App => {
                                let app =
                                    Z3_to_app(*z3_ctx, inner_int).expect("Z3_to_app returned null");
                                let decl = Z3_get_app_decl(*z3_ctx, app)
                                    .expect("Z3_get_app_decl returned null");
                                let decl_kind = Z3_get_decl_kind(*z3_ctx, decl);
                                ...
                                match decl_kind {
                                    ...
                                    DeclKind::SEQ_INDEX => {
                                        ...
                                        let arg2 = RcAst::try_from(
                                            Z3_get_app_arg(*z3_ctx, app, 2)
                                                .expect("Z3_get_app_arg returned null"),
                                        )?;
                                        let haystack = StringAst::from_z3(ctx, arg0)?;
                                        let needle = StringAst::from_z3(ctx, arg1)?;
                                        // offset is an int, convert to bv
                                        let offset_bv = RcAst::try_from(
                                            Z3_mk_int2bv(*z3_ctx, 64, *arg2)
                                                .expect("Z3_mk_int2bv returned null"),
                                        )?;
                                        let offset_simplified = RcAst::try_from(
                                            Z3_simplify(*z3_ctx, *offset_bv)
                                                .expect("Z3_simplify returned null"),
                                        )?;
