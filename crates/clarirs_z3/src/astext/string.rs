...
fn mk_bv2int(bv: &RcAst) -> Result<RcAst, ClarirsError> {
    Z3_CONTEXT.with(|&z3_ctx| unsafe {
        RcAst::try_from(Z3_mk_bv2int(z3_ctx, **bv, false).expect("Z3_mk_bv2int returned null"))
    })
}
    ...
    Z3_CONTEXT.with(|&z3_ctx| unsafe {
        Ok(match ast.op() {
            StringOp::StringS(s) => {
                ...
                let sort = Z3_mk_string_sort(z3_ctx).expect("Z3_mk_string_sort returned null");
            ...
            StringOp::StringV(s) => {
                ...
                RcAst::try_from(
                    Z3_mk_string(z3_ctx, cstr.as_ptr()).expect("Z3_mk_string returned null"),
                )?
            }
            StringOp::StrConcat(..) => {
                ...
                RcAst::try_from(
                    Z3_mk_seq_concat(z3_ctx, 2, [**a, **b].as_ptr())
                        .expect("Z3_mk_seq_concat returned null"),
                )?
            }
            StringOp::StrSubstr(..) => {
                let a = child(children, 0)?;
                ...
                RcAst::try_from(
                    Z3_mk_seq_extract(z3_ctx, **a, *offset_int, *len_int)
                        .expect("Z3_mk_seq_extract returned null"),
                )?
            }
            StringOp::StrReplace(..) => {
                let a = child(children, 0)?;
                ...
                RcAst::try_from(
                    Z3_mk_seq_replace(z3_ctx, **a, **b, **c)
                        .expect("Z3_mk_seq_replace returned null"),
                )?
            }
            StringOp::BVToStr(_) => {
                let a = child(children, 0)?;
                ...
                RcAst::try_from(
                    Z3_mk_int_to_str(z3_ctx, *int_val).expect("Z3_mk_int_to_str returned null"),
                )?
            }
            StringOp::ITE(..) => {
            ...
            AstKind::App => {
                ...
                match decl_kind {
                    ...
                    _ if Z3_is_string(z3_ctx, *ast) => {
                        let string_ptr = Z3_get_string(z3_ctx, *ast);
                    ...
                    DeclKind::UNINTERPRETED => {
                        ...
                        if sort_kind != SortKind::Seq {
                            return Err(ClarirsError::ConversionError(
                                "expected a string".to_string(),
                            ));
                        }
                        let sym =
                            Z3_get_decl_name(z3_ctx, decl).expect("Z3_get_decl_name returned null");
                    ...
                    DeclKind::SEQ_EXTRACT => {
                        ...
                        let offset_bv = RcAst::try_from(
                            Z3_mk_int2bv(z3_ctx, 64, *arg1).expect("Z3_mk_int2bv returned null"),
                        )?;
                        let offset_simplified = RcAst::try_from(
                            Z3_simplify(z3_ctx, *offset_bv).expect("Z3_simplify returned null"),
                        )?;
                        let offset = BitVecAst::from_z3(ctx, offset_simplified)?;

                        let len_bv = RcAst::try_from(
                            Z3_mk_int2bv(z3_ctx, 64, *arg2).expect("Z3_mk_int2bv returned null"),
                        )?;
                        let len_simplified = RcAst::try_from(
                            Z3_simplify(z3_ctx, *len_bv).expect("Z3_simplify returned null"),
                        )?;
                    ...
                    DeclKind::INT_TO_STR => {
                        ...
                        let inner_app = Z3_to_app(z3_ctx, *arg0).expect("Z3_to_app returned null");
                        let inner_decl = Z3_get_app_decl(z3_ctx, inner_app)
                            .expect("Z3_get_app_decl returned null");
                        let inner_kind = Z3_get_decl_kind(z3_ctx, inner_decl);
                        if inner_kind == DeclKind::BV2INT {
                            let bv_arg = RcAst::try_from(
                                Z3_get_app_arg(z3_ctx, inner_app, 0)
