...
mod to_z3 {
    ...
    fn substr() {
        let ctx = Context::new();
        let s = ctx.stringv("hello world").unwrap();
        let start = ctx.bvv(BitVec::from(6u32)).unwrap();
        let length = ctx.bvv(BitVec::from(5u32)).unwrap();
    ...
    fn substr() {
        ...
        let expected = ctx
            .str_substr(
                ...
                ctx.bvv(BitVec::from(6u64)).unwrap(),
                ctx.bvv(BitVec::from(5u64)).unwrap(),
            )
            .unwrap();
        assert_eq!(expected, result);
    }
...
mod roundtrip {
    ...
    fn substr_value() {
        let ctx = Context::new();
        let s = ctx.stringv("hello world").unwrap();
        let start = ctx.bvv(BitVec::from(6u64)).unwrap();
        let length = ctx.bvv(BitVec::from(5u64)).unwrap();
    ...
    fn substr_symbol() {
        ...
        let start = ctx.bvv(BitVec::from(0u64)).unwrap();
        let length = ctx.bvv(BitVec::from(3u64)).unwrap();
        let ast = ctx.str_substr(s, start, length).unwrap();
        assert_eq!(ast, round_trip(&ctx, &ast).unwrap());
    }
    ...
    fn bv_to_str_value() {
        let ctx = Context::new();
        let bv = ctx.bvv(BitVec::from(42u64)).unwrap();
        let ast = ctx.bv_to_str(bv).unwrap();
        assert_eq!(ast, round_trip(&ctx, &ast).unwrap());
    }
}
