...
mod to_z3 {
    ...
    fn concat_symbols() {
        ...
        assert_eq!(z3_ast.decl().kind(), z3::DeclKind::SeqConcat);
        assert_eq!(z3_ast.num_children(), 2);
        assert_eq!(arg_sym(&z3_ast, 0).as_deref(), Some("a"));
        assert_eq!(arg_sym(&z3_ast, 1).as_deref(), Some("b"));
    }
...
mod from_z3 {
    ...
    fn value_simple() {
        let ctx = Context::new();
        let z3_ast = mk_string_val("hello");
        let result = AstRef::from_z3(&ctx, z3_ast).unwrap();
    ...
    fn substr() {
        let ctx = Context::new();
        let s = mk_string_val("hello world");
        let start = z3hl::ast::Dynamic::from_ast(&z3hl::ast::Int::from_i64(6));
        let len = z3hl::ast::Dynamic::from_ast(&z3hl::ast::Int::from_i64(5));
