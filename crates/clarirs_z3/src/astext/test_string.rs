...
mod to_z3 {
    ...
    fn concat() {
        ...
        assert_eq!(z3_ast.decl_kind(), DeclKind::SEQ_CONCAT);
    ...
    fn substr() {
        ...
        assert_eq!(z3_ast.decl_kind(), DeclKind::SEQ_EXTRACT);
    ...
    fn replace() {
        ...
        assert_eq!(z3_ast.decl_kind(), DeclKind::SEQ_REPLACE);
        assert_eq!(z3_ast.num_args(), 3);
        assert_z3_string_value(&z3_ast.arg(0).unwrap(), "hello world");
        assert_z3_string_value(&z3_ast.arg(1).unwrap(), "world");
...
mod from_z3 {
    ...
    fn concat() {
        ...
        let z3_cat = Z3_CONTEXT.with(|&z3_ctx| unsafe {
            let args = [*s1, *s2];
            RcAst::try_from(
                Z3_mk_seq_concat(z3_ctx, 2, args.as_ptr()).expect("Z3_mk_seq_concat returned null"),
            )
            .unwrap()
        });
    ...
    fn substr() {
        ...
        let z3_sub = Z3_CONTEXT.with(|&z3_ctx| unsafe {
            ...
            let start = Z3_mk_numeral(z3_ctx, start_cstr.as_ptr(), int_sort)
                .expect("Z3_mk_numeral returned null");
            let len_cstr = std::ffi::CString::new("5").unwrap();
            let len = Z3_mk_numeral(z3_ctx, len_cstr.as_ptr(), int_sort)
                .expect("Z3_mk_numeral returned null");
            RcAst::try_from(
                Z3_mk_seq_extract(z3_ctx, *s, start, len).expect("Z3_mk_seq_extract returned null"),
            )
            .unwrap()
        });
    ...
    fn replace() {
        ...
        let z3_rep = Z3_CONTEXT.with(|&z3_ctx| unsafe {
            RcAst::try_from(
                Z3_mk_seq_replace(z3_ctx, *s, *pat, *rep).expect("Z3_mk_seq_replace returned null"),
            )
            .unwrap()
        });
    ...
    fn ite() {
        ...
        let z3_ite = Z3_CONTEXT.with(|&z3_ctx| unsafe {
            RcAst::try_from(Z3_mk_ite(z3_ctx, *c, *then, *else_).expect("Z3_mk_ite returned null"))
                .unwrap()
        });
