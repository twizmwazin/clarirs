...
fn test_leading_zeros() -> Result<(), BitVecError> {
    ...
    let bv = BitVec::from((0b010u8, 3)); // Only 3 bits: 010
...
fn test_is_zero() -> Result<(), BitVecError> {
    ...
    let bv = BitVec::zero(1);
    assert!(bv.is_zero());
    let bv = BitVec::from((1u64, 1));
    assert!(!bv.is_zero());

    // Test odd-sized vectors
    let bv = BitVec::zero(3);
    assert!(bv.is_zero());
...
fn test_reverse_bytes() -> Result<(), BitVecError> {
    // Test single byte
    let bv = BitVec::from((0xA5u8, 8)); // 10100101
    ...
    let bv = BitVec::from((0b10110u8, 5)); // 5 bits
    assert!(bv.reverse_bytes().is_err()); // Should fail as width is not byte-aligned
...
fn test_serde_roundtrip_recomputes_final_word_mask() -> Result<(), BitVecError> {
    ...
    let original = BitVec::from((0xFFu8, 8));
