    ...
    fn simplify(&self) -> Result<Self, ClarirsError> {
        ...
        ctx.simplification_cache.get_or_insert_with_bv(hash, || {
            match &self.op() {
                ...
                //             // Check if the sign bit is set (MSB == 1)
                //             let sign_bit_set = (unsigned_value.clone() >> (bit_length - 1))
                //                 & BigUint::one()
                //                 != BigUint::zero();

                //             // Handle excessive shift amounts
                //             if shift_amount_usize >= bit_length {
                //                 let result = if sign_bit_set {
                //                     // All ones for negative values
                //                     (BigUint::one() << bit_length) - BigUint::one()
                //                 } else {
                //                     // All zeros for positive values
                //                     BigUint::zero()
                //                 };
                //                 return ctx.bvv(BitVec::from_biguint_trunc(&result, bit_length));
                //             }
                ...
                //             let unsigned_value = value.to_biguint();
                //             let sign_bit_set = (unsigned_value.clone() >> (bit_length - 1)) & BigUint::one() != BigUint::zero();

                //             if shift_amount_usize >= bit_length {
                //                 return if sign_bit_set {
                //                     // All ones for negative values
                //                     ctx.bvv(BitVec::from_biguint_trunc(&((BigUint::one() << bit_length) - BigUint::one()), bit_length))
                //                 } else {
                //                     // All zeros for positive values
                //                     ctx.bvv(BitVec::zeros(bit_length))
                //                 };
                ...
                //             let unsigned_shifted = unsigned_value.clone() >> shift_amount_usize;
                //             let result = if sign_bit_set {
                //                 let mask = (BigUint::one() << (bit_length - shift_amount_usize)) - BigUint::one();
                //                 unsigned_shifted | (mask << (bit_length - shift_amount_usize))
                //             } else {
                //                 unsigned_shifted
                //             };
                ...
                //         (BitVecOp::BVV(value), BitVecOp::BVV(shift_amount)) => {
                //             let bit_width = value.len();
                //             let shift_amount_usize = shift_amount.to_usize().unwrap_or(0).min(bit_width);
                //             let result = if value.is_negative() {
                //                 // Treat as signed for negative values
                //                 ((value.clone() as i64) >> shift_amount_usize) as u64
                //             } else {
                //                 value.clone() >> shift_amount_usize
                //             };
                //             ctx.bvv(result)
