...
// Helper function to extract a child from transformed children
pub fn child<'c>(children: &[AstRef<'c>], index: usize) -> Result<AstRef<'c>, ClarirsError> {
    children
        .get(index)
        .cloned()
...
mod tests {
    ...
    fn test_walk_post_order_with_cache() -> Result<(), ClarirsError> {
        ...
        assert_eq!(
            first_visited,
            vec![x.clone(), y.clone(), add1.clone(), mul.clone()],
        );
