...
pub(crate) fn excavate_ite<'c>(
    ...
    match ast.op() {
        ...
        Op::StrSubstr(..) => {
            ...
            } else if let Op::ITE(cond, then_, else_) = start.op() {
                Ok(ctx.ite(
                    cond,
                    ctx.str_substr(&base, then_, &size)?,
                    ctx.str_substr(base, else_, size)?,
                )?)
            } else if let Op::ITE(cond, then_, else_) = size.op() {
                Ok(ctx.ite(
                    cond,
        ...
        Op::StrReplace(..) => {
            ...
            } else if let Op::ITE(cond, then_, else_) = pattern.op() {
                Ok(ctx.ite(
                    cond,
                    ctx.str_replace(&base, then_, &replacement)?,
                    ctx.str_replace(base, else_, replacement)?,
                )?)
            } else if let Op::ITE(cond, then_, else_) = replacement.op() {
                Ok(ctx.ite(
                    cond,
