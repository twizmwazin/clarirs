...
fn test_bitvec_neg_with_ite() {
    ...
    let expected = ctx.if_(&c, &neg_a, &neg_b).unwrap();
...
fn test_bitvec_add_with_ite() {
    ...
    let expected = ctx.if_(&c, &d_add_a, &d_add_b).unwrap();
...
fn test_bitvec_sub_with_ite() {
    ...
    let expected = ctx.if_(&c, &d_sub_a, &d_sub_b).unwrap();
...
fn test_bitvec_mul_with_ite() {
    ...
    let expected = ctx.if_(&c, &d_mul_a, &d_mul_b).unwrap();
