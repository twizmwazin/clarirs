...
pub(crate) fn excavate_ite<'c>(
    ...
    match ast.op() {
        ...
        AstOp::FpFP(..) => {
            ...
            // Check sign for ITE
            if let AstOp::ITE(cond, then_, else_) = sign.op() {
                Ok(ctx.ite(
                    cond,
                    ctx.fp_fp(then_, &exponent, &significand)?,
                    ctx.fp_fp(else_, exponent, significand)?,
                )?)
            } else if let AstOp::ITE(cond, then_, else_) = exponent.op() {
                Ok(ctx.ite(
                    cond,
                    ctx.fp_fp(&sign, then_, &significand)?,
                    ctx.fp_fp(sign, else_, significand)?,
                )?)
            } else if let AstOp::ITE(cond, then_, else_) = significand.op() {
                Ok(ctx.ite(
                    cond,
        ...
        _ => unreachable!("non-float op dispatched to excavate_ite"),
    }
}
