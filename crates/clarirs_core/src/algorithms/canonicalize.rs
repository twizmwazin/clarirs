...
///
/// assert!(structurally_match(&ast1, &ast2)?);
/// # Ok::<(), ClarirsError>(())
/// ```
...
pub fn canonicalize<'c>(
    ...
    let var_mapping: HashMap<InternedString, InternedString> = var_names
        .iter()
        .enumerate()
        .map(|(i, name)| (name.clone(), ctx.intern_string(format!("v{i}"))))
        .collect();
    ...
    for var in vars {
            ...
            && let Some(canonical_name) = var_mapping.get(original_name)
        {
            ...
            let canonical_var = match var.op() {
                AstOp::BoolS(_) => ctx.bools(canonical_name.as_str())?,
                AstOp::BVS(_, size) => ctx.bvs(canonical_name.as_str(), *size)?,
                AstOp::FPS(_, sort) => ctx.fps(canonical_name.as_str(), *sort)?,
                AstOp::StringS(_) => ctx.strings(canonical_name.as_str())?,
                _ => var.clone(), // non-variable leaf, keep as-is
            };
...
mod tests {
    ...
    fn test_canonicalize_bitvec() -> Result<(), ClarirsError> {
        ...
        let (map1, counter1, canonical1) = canonicalize(&ast1)?;
        let (map2, counter2, canonical2) = canonicalize(&ast2)?;
    ...
    fn test_canonicalize_complex() -> Result<(), ClarirsError> {
        ...
        let (_, counter1, canonical1) = canonicalize(&ast1)?;
        let (_, counter2, canonical2) = canonicalize(&ast2)?;
    ...
    fn test_canonicalize_no_vars() -> Result<(), ClarirsError> {
        ...
        let (map, counter, canonical) = canonicalize(&ast)?;

        // Should be unchanged
        assert_eq!(ast, canonical);
        assert!(map.is_empty());
        assert_eq!(counter, 0);
    ...
    fn test_canonicalize_order_independence() -> Result<(), ClarirsError> {
        ...
        assert_eq!(canonical1, expected1);
    ...
    fn test_structurally_match_same_ast() -> Result<(), ClarirsError> {
        ...
        assert!(structurally_match(&ast, &ast)?);
    ...
    fn test_structurally_match_constants() -> Result<(), ClarirsError> {
        ...
        assert!(!structurally_match(&ast1, &ast3)?);
