...
pub fn canonicalize<'c>(ast: &DynAst<'c>) -> Result<DynAst<'c>, ClarirsError> {
    ...
    let mut var_names: Vec<String> = vars.iter()
        ...
        if let Some(original_name) = var_names_set.iter().next() {
            if let Some(canonical_name) = var_mapping.get(original_name) {
                ...
                let canonical_var = match &var {
                    DynAst::Boolean(_) => {
                        DynAst::Boolean(ctx.bools(canonical_name)?)
                    }
                    DynAst::BitVec(bv) => {
                        let size = bv.size();
                    ...
                    DynAst::String(_) => {
                        DynAst::String(ctx.strings(canonical_name)?)
                    }
                };
...
mod tests {
    ...
    fn test_canonicalize_no_vars() -> Result<(), ClarirsError> {
        ...
        let ast = ctx.add(&ctx.bvv_prim_with_size(5u64, 32)?, &ctx.bvv_prim_with_size(10u64, 32)?)?;
    ...
    fn test_canonicalize_single_var() -> Result<(), ClarirsError> {
        ...
        let canonical_expected = ctx.add(&ctx.bvs("v0", 64)?, &ctx.bvv_prim_with_size(5u64, 64)?)?;
    ...
    fn test_structurally_match_boolean() -> Result<(), ClarirsError> {
        ...
        // Create two structurally identical boolean ASTs with different variable names
        let ast1 = ctx.or(&ctx.and(&ctx.bools("a")?, &ctx.bools("b")?)?, &ctx.bools("c")?)?;
        let ast2 = ctx.or(&ctx.and(&ctx.bools("x")?, &ctx.bools("y")?)?, &ctx.bools("z")?)?;
