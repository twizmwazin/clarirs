...
fn test_neg() -> Result<()> {
    ...
    for (a, expected) in table.clone() {
        let a = ctx.bvv(BitVec::from(a)).unwrap();
        let expected = ctx.bvv(BitVec::from(expected)).unwrap();
...
fn test_add() -> Result<()> {
    ...
    for (a, b, expected) in table {
        let a = ctx.bvv(BitVec::from(a)).unwrap();
        let b = ctx.bvv(BitVec::from(b)).unwrap();
        let expected = ctx.bvv(BitVec::from(expected)).unwrap();
...
fn test_sdiv() -> Result<()> {
    ...
    for (a_i64, b_i64, expected_i64) in table {
        ...
        let a = ctx.bvv(BitVec::from(a_bits))?;
        let b = ctx.bvv(BitVec::from(b_bits))?;
        let expected = ctx.bvv(BitVec::from(expected_bits))?;
...
fn test_shl() -> Result<()> {
    ...
    let x = ctx.bvs("x", 64)?;
    let zero = ctx.bvv(BitVec::from(0u64))?;
...
fn test_zext() -> Result<()> {
    ...
    let table = vec![
        ...
        (
            ctx.bvv(BitVec::from((0u8, 4)))?,
            1,
            ctx.bvv(BitVec::from((0u8, 5)))?,
        ),
        (
            ctx.bvv(BitVec::from((1u8, 4)))?,
            0,
            ctx.bvv(BitVec::from((1u8, 4)))?,
        ),
        (
...
fn test_sext() -> Result<()> {
    ...
    let table = vec![
        ...
        (
            ctx.bvv(BitVec::from((15u8, 4)))?,
            1,
            ctx.bvv(BitVec::from((31u8, 5)))?,
        ),
        (
            ctx.bvv(BitVec::from((0u8, 1)))?,
            1,
            ctx.bvv(BitVec::from((0u8, 2)))?,
        ),
        (
            ctx.bvv(BitVec::from((1u8, 1)))?,
            1,
            ctx.bvv(BitVec::from((3u8, 2)))?,
        ),
        (
            ctx.bvv(BitVec::from((8u8, 4)))?,
            4,
            ctx.bvv(BitVec::from((248u8, 8)))?,
        ),
        (
            ctx.bvv(BitVec::from((5u8, 4)))?,
            4,
            ctx.bvv(BitVec::from((5u8, 8)))?,
        ),
    ];
...
fn test_byte_reverse() -> Result<()> {
    ...
    for (a, expected) in table {
        let a = context.bvv(BitVec::from(a)).unwrap();
        let expected = context.bvv(BitVec::from(expected)).unwrap();
...
fn test_extract() -> Result<()> {
    ...
    let bv = ctx.bvv(BitVec::from(0x1234_5678_9ABC_DEF0_u64)).unwrap();
    ...
    let expected = ctx.bvv(BitVec::from(0x1234_5678_u32)).unwrap();
...
fn test_identity_simplifications() -> anyhow::Result<()> {
    ...
    let zero = ctx.bvv(BitVec::from(0u64))?;
    let one = ctx.bvv(BitVec::from(1u64))?;
    let all_ones = ctx.bvv(BitVec::from(u64::MAX))?;
...
fn test_extract_full_width() -> Result<()> {
    ...
    let bvv = ctx.bvv(BitVec::from(42u32))?;
