...
/// Rebuild a DynAst node from its (possibly replaced) children.
///
/// This is the shared rebuild logic used by both single-replacement and
/// multi-replacement operations. Given a node and its already-transformed
/// children, it reconstructs the node using the context's factory methods.
pub(crate) fn rebuild_ast<'c>(
    ctx: &'c Context<'c>,
    node: &DynAst<'c>,
    children: &[DynAst<'c>],
...
/// Apply all replacements from a hash map in a single post-order pass.
///
/// Each node whose `inner_hash()` appears in the map is replaced with the
/// corresponding value. Children are processed before parents, so replacements
/// compose naturally.
pub fn replace_all<'c>(
    ast: DynAst<'c>,
