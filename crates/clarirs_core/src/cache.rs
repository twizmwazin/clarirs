...
/// A trait for caching values based on a key.
pub trait Cache<K, V> {
...
impl<K, V> Cache<K, V> for () {
    fn get_or_insert<E>(&self, _: K, value_cv: impl FnOnce() -> Result<V, E>) -> Result<V, E> {
        value_cv()
    }
...
/// A special cache for AST nodes, storing weak references.
