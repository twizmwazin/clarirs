//! Python bindings for clarirs, gated behind the `python` feature.
//!
//! This module hosts the PyO3 glue that was previously the standalone
//! `clarirs_py` crate. Living inside `clarirs_core` lets the conversion traits
//! (`IntoPyObject`/`FromPyObject`) be implemented directly on the core AST types
//! without tripping the orphan rule.
//!
//! The solver and VSA bindings cannot live here: they depend on `clarirs_z3`
//! and `clarirs-vsa`, which in turn depend on `clarirs_core`, so keeping them
//! here would form a dependency cycle. Those remain in the thin `clarirs_py`
//! crate, which builds the `claripy` extension module by calling [`register`]
//! and then adding its own solver/VSA pieces.
...
/// Create a submodule, populate it with `import_func`, and register it under
/// `sys.modules` so `import package.name` works.
pub fn import_submodule<'py>(
    py: Python<'py>,
    m: &Bound<'py, PyModule>,
    package: &str,
    name: &str,
...
/// Build a submodule with `build_func`, register it under `sys.modules`, and
/// expose it as an attribute of `m`.
...
/// Register everything that does not depend on solvers or VSA into the given
/// module. The `clarirs_py` crate calls this from its `#[pymodule]` and then
/// adds the solver and VSA bindings on top.
