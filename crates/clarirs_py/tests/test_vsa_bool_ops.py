...
class TestVSABoolOperations(unittest.TestCase):
    def setUp(self):
        ...
        # Create VSA solver
        self.solver = clarirs.solver.VSASolver()

        # Create concrete boolean values
        self.true_val = clarirs.BoolV(True)
        self.false_val = clarirs.BoolV(False)

        # Create symbolic boolean values
        self.sym_bool1 = clarirs.BoolS("x")
        self.sym_bool2 = clarirs.BoolS("y")

        # Create BVs for comparison testing
        self.bv_const1 = clarirs.BVV(10, 32)
        self.bv_const2 = clarirs.BVV(5, 32)
        self.bv_sym = clarirs.BVS("z", 32)

        # Create strided intervals for testing
        self.si_single = clarirs.SI(bits=32, stride=0, lower_bound=10, upper_bound=10)
        self.si_range = clarirs.SI(bits=32, stride=1, lower_bound=5, upper_bound=15)
        self.si_stride2 = clarirs.SI(bits=32, stride=2, lower_bound=0, upper_bound=10)
        ...
        self.si_min = clarirs.SI(bits=32, stride=0, lower_bound=0, upper_bound=0)
    ...
    def test_comparison_ranges(self):
        ...
        # [5, 15] > 3
        self.assertTrue(self.solver.is_true(self.si_range > clarirs.BVV(3, 32)))
        # [5, 15] >= 5
        self.assertTrue(self.solver.is_true(self.si_range >= clarirs.BVV(5, 32)))
        # [5, 15] < 20
        self.assertTrue(self.solver.is_true(self.si_range < clarirs.BVV(20, 32)))
        # [5, 15] <= 15
        self.assertTrue(self.solver.is_true(self.si_range <= clarirs.BVV(15, 32)))

        # Test ranges where the result is maybe
        # [5, 15] == 10
        range_eq = self.si_range == clarirs.BVV(10, 32)
        self.assertTrue(self.solver.has_true(range_eq))
        self.assertTrue(self.solver.has_false(range_eq))
        # [5, 15] != 10
        range_ne = self.si_range != clarirs.BVV(10, 32)
        self.assertTrue(self.solver.has_true(range_ne))
        self.assertTrue(self.solver.has_false(range_ne))
        # [5, 15] > 10
        range_gt = self.si_range > clarirs.BVV(10, 32)
        self.assertTrue(self.solver.has_true(range_gt))
        self.assertTrue(self.solver.has_false(range_gt))
        # [5, 15] < 10
        range_lt = self.si_range < clarirs.BVV(10, 32)
    ...
    def test_signed_comparison(self):
        ...
        # [-5, 5] <s 10
        self.assertTrue(self.solver.is_true(self.si_mixed.SLT(clarirs.BVV(10, 32))))
        # [-5, 5] <s 0
        mixed_slt = self.si_mixed.SLT(clarirs.BVV(0, 32))
        self.assertTrue(self.solver.has_true(mixed_slt))
        self.assertTrue(self.solver.has_false(mixed_slt))
        # [-5, 5] >s -10
        self.assertTrue(self.solver.is_true(self.si_mixed.SGT(clarirs.BVV(-10, 32))))
        # [-5, 5] >s 0
        mixed_sgt = self.si_mixed.SGT(clarirs.BVV(0, 32))
    ...
    def test_basic_if_then_else(self):
        ...
        # If(True, 10, 5) == 10
        result1 = clarirs.If(self.true_val, self.bv_const1, self.bv_const2)
        vals = self.solver.eval(result1, 10)
        self.assertEqual(len(vals), 1)
        self.assertEqual(vals[0], 10)

        # If(False, 10, 5) == 5
        result2 = clarirs.If(self.false_val, self.bv_const1, self.bv_const2)
    ...
    def test_if_with_bv_condition(self):
        ...
        result = clarirs.If(cond, self.bv_const1, self.bv_const2)
        ...
        cond = self.si_range == clarirs.BVV(10, 32)
        result = clarirs.If(cond, self.bv_const1, self.bv_const2)
