...
impl<'c, A: Solver<'c>, E: Solver<'c>> Solver<'c> for HybridSolver<'c, A, E> {
    ...
    fn constraints(&self) -> Result<Vec<BoolAst<'c>>, ClarirsError> {
        // The exact solver is the source of truth for constraints
    ...
    fn satisfiable(&mut self) -> Result<bool, ClarirsError> {
        ...
        match self.approximate.satisfiable() {
    ...
    fn is_true(&mut self, expr: &BoolAst<'c>) -> Result<bool, ClarirsError> {
        // If the expression is concrete, no need to consult solvers
        if !expr.symbolic() {
            return self.approximate.is_true(expr);
        }
        // Try approximate first
        match self.approximate.is_true(expr) {
            Ok(true) => {
                // Approximate says definitely true - verify with exact
                // (approximate solvers may over-approximate)
                self.exact.is_true(expr)
            }
            Ok(false) => {
                // Approximate says not definitely true, but it might be
                // imprecise, so check exact
                self.exact.is_true(expr)
            }
            Err(_) => self.exact.is_true(expr),
        }
    }
    ...
    fn is_false(&mut self, expr: &BoolAst<'c>) -> Result<bool, ClarirsError> {
        ...
        match self.approximate.is_false(expr) {
            Ok(true) => self.exact.is_false(expr),
            Ok(false) => self.exact.is_false(expr),
            Err(_) => self.exact.is_false(expr),
        }
    }
    ...
    fn has_true(&mut self, expr: &BoolAst<'c>) -> Result<bool, ClarirsError> {
        ...
        // If approximate says no possibility of true, we can try exact
        // If approximate says maybe true, verify with exact
        match self.approximate.has_true(expr) {
            Ok(false) => {
                // Approximate says definitely no true - but approximate solvers
                // can under-approximate, so check exact
                self.exact.has_true(expr)
            }
            Ok(true) => Ok(true),
            Err(_) => self.exact.has_true(expr),
        }
    }
    ...
    fn has_false(&mut self, expr: &BoolAst<'c>) -> Result<bool, ClarirsError> {
        ...
        match self.approximate.has_false(expr) {
            Ok(false) => self.exact.has_false(expr),
            Ok(true) => Ok(true),
            Err(_) => self.exact.has_false(expr),
        }
    }
    ...
    fn min_unsigned(&mut self, expr: &BitVecAst<'c>) -> Result<BitVecAst<'c>, ClarirsError> {
        ...
        // The exact solver gives the precise minimum; approximate gives a bound.
        // Use exact for correctness.
    ...
    fn eval_bool_n(
        ...
        // For concrete expressions, the approximate solver suffices
        if !expr.symbolic() {
            return self.approximate.eval_bool_n(expr, n);
        }
        // Try approximate first for speed
        match self.approximate.eval_bool_n(expr, n) {
            Ok(approx_results) if !approx_results.is_empty() => {
                // Validate approximate results against exact solver:
                // check if the approximate results are actually feasible
                // under the current constraints.
                let exact_results = self.exact.eval_bool_n(expr, n);
                match exact_results {
                    Ok(exact) => Ok(exact),
    ...
    fn eval_bitvec_n(
        ...
        match self.approximate.eval_bitvec_n(expr, n) {
            Ok(approx_results) if !approx_results.is_empty() => {
                // Fall back to exact for precise results under constraints
    ...
    fn eval_float_n(
        ...
        // Float is often unsupported by approximate solvers, go straight to exact
    ...
    fn eval_string_n(
        ...
        // String is often unsupported by approximate solvers, go straight to exact
...
mod tests {
    ...
    fn test_hybrid_solver_concrete() -> Result<(), ClarirsError> {
        let ctx = Context::new();
        // Use two ConcreteSolvers as a basic smoke test
    ...
    #[test]
    fn test_hybrid_solver_constraints() -> Result<(), ClarirsError> {
        let ctx = Context::new();
        let approx = ConcreteSolver::new(&ctx);
        let exact = ConcreteSolver::new(&ctx);
        let mut solver = HybridSolver::new(&ctx, approx, exact);

        // Adding constraints should succeed (they go to exact solver)
        let constraint = ctx.true_()?;
        solver.add(&constraint)?;

        // Constraints come from exact solver
        let constraints = solver.constraints()?;
        assert!(constraints.is_empty()); // ConcreteSolver ignores constraints

        Ok(())
    }

    #[test]
    fn test_hybrid_solver_clear() -> Result<(), ClarirsError> {
        let ctx = Context::new();
        let approx = ConcreteSolver::new(&ctx);
        let exact = ConcreteSolver::new(&ctx);
        let mut solver = HybridSolver::new(&ctx, approx, exact);
