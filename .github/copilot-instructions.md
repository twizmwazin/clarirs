...
`clarirs_z3` depends on the `z3-sys` crate (with `bundled` feature) for low-level Z3 FFI bindings. A thin compatibility shim (`z3_compat.rs`) provides short names for z3-sys symbols.
